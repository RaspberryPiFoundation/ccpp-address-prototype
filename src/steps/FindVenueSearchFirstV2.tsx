...
/**
 * Search-first (v2). Starts life as an exact copy of FindVenueSearchFirst — the
 * working copy we're iterating on, so the original stays untouched for comparison.
 * Only the address search shows to begin with; once a search returns a result the
 * map appears so the user can fine-tune the pin. There's no coordinate entry.
 * Clicking "Confirm venue location" reverse-geocodes the pin, reveals the address
 * for confirmation, and freezes the map.
 */
export function FindVenueSearchFirstV2({
  data,
  coordinates,
  update,
  updateAddress,
